...
<body>
	...
	<nav aria-label="breadcrumb">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active"><a href="administrator.jsp">Zona de Administración</a></li>
		</ol>
	</nav>
								...
								<c:when test="${role=='admin'}">
									<form action="ChangeStatusServlet" method="post">
										<input type="hidden"
											...
											name="product_id"
											value="<c:out value="${item.getProduct_id()}"/>&unapprove">
					...
					<div class="modal-content">
						...
						<div class="modal-footer">
							...
							<c:choose>
								<c:when test="${role=='admin'}">
												<form action="ChangeStatusServlet" method="post">
													<input type="hidden" id="<c:out value="${item.getProduct_id()}"/>" name="product_id" value="<c:out value="${item.getProduct_id()}"/>&approve">										
													<button style="margin-right: 10px" class="nav navbar-nav navbar-right btn btn-warning" type="submit"
															style="margin-rigth: 20px"> <i class="icon-thumbs-up"> </i> <fmt:message key="products.approveproduct"/></button>
												</form>
											</c:when>
								<c:when test="${role=='admin'}">
									...
									<form action="RemoveServlet" method="post">
										<input type="hidden"
											id="<c:out value="${item.getProduct_id()}"/>"
											name="product_id"
											value="<c:out value="${item.getProduct_id()}"/>">
										<button style="margin-right: 10px"
											class="nav navbar-nav navbar-right btn btn-danger"
											type="submit" style="margin-rigth: 20px">
